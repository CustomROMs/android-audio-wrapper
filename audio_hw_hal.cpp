...
uint32_t legacy_in_get_sample_rate(const struct audio_stream *stream)
{
...
int legacy_in_set_sample_rate(struct audio_stream *stream, uint32_t rate)
{
...
size_t legacy_in_get_buffer_size(const struct audio_stream *stream)
{
    const struct legacy_stream_in *in =
        reinterpret_cast<const struct legacy_stream_in *>(stream);
...
static audio_channel_mask_t legacy_in_get_channels(const struct audio_stream *stream)
{
    const struct legacy_stream_in *in =
        reinterpret_cast<const struct legacy_stream_in *>(stream);
...
static audio_format_t legacy_in_get_format(const struct audio_stream *stream)
{
    const struct legacy_stream_in *in =
        reinterpret_cast<const struct legacy_stream_in *>(stream);
    // legacy API, don't change return type
...
int legacy_in_set_format(struct audio_stream *stream, audio_format_t format)
{
    struct legacy_stream_in *in =
        reinterpret_cast<struct legacy_stream_in *>(stream);
    ALOGE("(%s:%d) %s: Implement me!", __FILE__, __LINE__, __func__);
...
int legacy_in_standby(struct audio_stream *stream)
{
    struct legacy_stream_in *in = reinterpret_cast<struct legacy_stream_in *>(stream);
    return in->legacy_in->standby();
}

int legacy_in_dump(const struct audio_stream *stream, int fd)
{
    const struct legacy_stream_in *in =
        reinterpret_cast<const struct legacy_stream_in *>(stream);
    Vector<String16> args;
...
int legacy_in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    struct legacy_stream_in *in =
        reinterpret_cast<struct legacy_stream_in *>(stream);
    int val;
...
char * legacy_in_get_parameters(const struct audio_stream *stream,
                                const char *keys)
{
    const struct legacy_stream_in *in =
        reinterpret_cast<const struct legacy_stream_in *>(stream);
...
int legacy_in_set_gain(struct audio_stream_in *stream, float gain)
{
    struct legacy_stream_in *in =
        reinterpret_cast<struct legacy_stream_in *>(stream);
...
ssize_t legacy_in_read(struct audio_stream_in *stream, void* buffer,
                       size_t bytes)
{
    struct legacy_stream_in *in =
        reinterpret_cast<struct legacy_stream_in *>(stream);
...
uint32_t legacy_in_get_input_frames_lost(struct audio_stream_in *stream)
{
    struct legacy_stream_in *in =
        reinterpret_cast<struct legacy_stream_in *>(stream);
...
int legacy_in_add_audio_effect(const struct audio_stream *stream, effect_handle_t effect)
{
    const struct legacy_stream_in *in =
        reinterpret_cast<const struct legacy_stream_in *>(stream);
...
int legacy_in_remove_audio_effect(const struct audio_stream *stream, effect_handle_t effect)
{
    const struct legacy_stream_in *in =
        reinterpret_cast<const struct legacy_stream_in *>(stream);
                                  ...
                                  audio_source_t source __unused)
{
    ...
    in->stream.common.get_sample_rate = legacy_in_get_sample_rate;
    in->stream.common.set_sample_rate = legacy_in_set_sample_rate;
    in->stream.common.get_buffer_size = legacy_in_get_buffer_size;
    in->stream.common.get_channels = legacy_in_get_channels;
    in->stream.common.get_format = legacy_in_get_format;
    in->stream.common.set_format = legacy_in_set_format;
    in->stream.common.standby = legacy_in_standby;
    in->stream.common.dump = legacy_in_dump;
    in->stream.common.set_parameters = legacy_in_set_parameters;
    in->stream.common.get_parameters = legacy_in_get_parameters;
    in->stream.common.add_audio_effect = legacy_in_add_audio_effect;
    in->stream.common.remove_audio_effect = legacy_in_remove_audio_effect;
    in->stream.set_gain = legacy_in_set_gain;
    in->stream.read = legacy_in_read;
    in->stream.get_input_frames_lost = legacy_in_get_input_frames_lost;
                            ...
                            hw_device_t** device)
{
    ...
    ladev->device.get_input_buffer_size = legacy_adev_get_input_buffer_size;
    ladev->device.open_output_stream = adev_open_output_stream;
    ladev->device.close_output_stream = adev_close_output_stream;
    ladev->device.open_input_stream = legacy_adev_open_input_stream;
    ladev->device.close_input_stream = legacy_adev_close_input_stream;
