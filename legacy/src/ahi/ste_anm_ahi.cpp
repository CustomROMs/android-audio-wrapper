...
void ril_ringback_callback(void *ANM, int unknown) {
	AudioHardwareANM *thiz = (AudioHardwareANM *)ANM;

	thiz->setRingbackMute(thiz, unknown - 1 + ((unsigned int)(unknown - 1) <= 0) - (unknown - 1), 0);
}
...
int AudioHardwareANM::setRingbackMute(AudioHardwareANM *thiz, bool unknown1, int unknown2) {
	return 0;
}
